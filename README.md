...
- Fit、100%、光标中心缩放、平移、坐标与像素检查：不显示 Alpha；YUV 同时给出原始 YUV 与转换后的 RGB，RAW 在未去马赛克时画面与像素值都来自马赛克本身（灰度）、去马赛克后给出与 RAW 位深一致的 RGB，16-bit 数据按原始位深显示；相机 RAW（LibRaw）按 16-bit 解码并保留传感器马赛克，参数编辑器里的去马赛克开关同时作用于画面与像素值；RAW/YUV 预览在悬停时会自动补齐完整解码后再给出源采样值；高倍放大时悬停坐标与画面像素精确对应
